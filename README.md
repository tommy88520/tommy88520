...
接手一個雲端教學平台的後端維運,一半時間在救火、一半時間在把救火的東西變成不用再救。抓出一個藏了好幾個月的地雷:正式站跟測試站的部署路徑透過舊的 symlink 偷偷共用同一份檔案,兩邊部署會互相蓋掉對方,重建成完全獨立的部署路徑解決;另外從零生出一套資料庫 migration 機制(sequelize-cli)取代原本純手動下 SQL 的流程,CI/CD 也補了正式環境部署要人工核准這類保護機制,加上即時 process 監控跟異常通知。

> 架構判斷與問題排查是自己來,操作過程有搭配 AI 編碼工具(Claude Code)加速實作與驗證。
